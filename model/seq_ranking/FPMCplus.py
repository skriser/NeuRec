...
class FPMCplus(AbstractRecommender):
    def __init__(self, sess, dataset):
        ...
        config.read("conf/FPMCplus.properties")
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            self.embeddings_UI = tf.Variable(
            ...
            self.embeddings_IU = tf.Variable(
            ...
            self.embeddings_IL = tf.Variable(
                tf.truncated_normal(shape=[self.num_items, self.embedding_size], mean=0.0, stddev=0.01),
                name='embeddings_IL', dtype=tf.float32)
            self.embeddings_LI = tf.Variable(
                tf.truncated_normal(shape=[self.num_items, self.embedding_size], mean=0.0, stddev=0.01),
    ...
    def _attention_MLP(self, embeddings_UI_u, embeddings_IL_i, item_embedding_recent):
        with tf.name_scope("attention_MLP"):
            ...
            MLP_output = tf.matmul(tf.reshape(embedding_short, shape=[-1, 3 * self.embedding_size]),
                                   self.W) + self.b  # (b*n, e or 2*e) * (e or 2*e, w) + (1, w)
            MLP_output = tf.nn.tanh(MLP_output)

            A_ = tf.reshape(tf.matmul(MLP_output, self.h),
                            shape=[batch_users, self.high_order])  # (b*n, w) * (w, 1) => (None, 1) => (b, n)
    ...
    def _create_loss(self):
        with tf.name_scope("loss"):
            ...
            if self.ispairwise.lower() == "true":
                ...
                self.loss = learner.pointwise_loss(self.loss_function, self.labels, self.output) + self.reg_mf * (
                            tf.reduce_sum(tf.square(UI_u)) +
                            tf.reduce_sum(tf.square(IU_i)) +
                            tf.reduce_sum(tf.square(IL_i)) +
                            tf.reduce_sum(tf.square(LI_l)))
                ...
                if self.ispairwise.lower() == "true":
                    bat_users, bat_items_pos, bat_items_recents, bat_items_neg = \
                        data_gen._get_pairwise_batch_seqdata(user_input, item_input_pos,
                                                             item_input_recents, item_input_neg, num_batch,
                                                             self.batch_size)
                    ...
                    bat_users, bat_items, bat_items_recents, bat_lables = \
                        data_gen._get_pointwise_batch_seqdata(user_input,
                                                              item_input, item_input_recents, lables, num_batch,
                                                              self.batch_size)
                    feed_dict = {self.user_input: bat_users, self.item_input: bat_items,
                                 self.item_input_recents: bat_items_recents, self.labels: bat_lables}
    ...
    def predict(self, user_id, items):
        ...
        return self.sess.run(
            (self.output),
            feed_dict={self.user_input: users, self.item_input_recents: item_recents, self.item_input: items}
        )
