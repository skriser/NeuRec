    ...
    def _create_loss(self):
        with tf.name_scope("loss"):
            ...
            if self.ispairwise.lower() == "true":
                ...
                self.loss = learner.pairwise_loss(self.loss_function, result) + self.reg_mf * (
                            tf.reduce_sum(tf.square(p1))
                            + tf.reduce_sum(tf.square(q2)) + tf.reduce_sum(tf.square(q1))) + self.reg_mlp * (
                                        tf.reduce_sum(tf.square(m1))
