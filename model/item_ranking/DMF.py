...
class DMF(AbstractRecommender):
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):  # The embedding initialization is unknown now
            self.u_w1 = tf.get_variable("u_w1", shape=(self.num_items, self.fist_layer_size),
                                        initializer=tf.contrib.layers.xavier_initializer())
            self.u_b1 = tf.get_variable("u_b1", shape=[self.fist_layer_size],
                                        initializer=tf.contrib.layers.xavier_initializer())
            self.u_w2 = tf.get_variable("u_w2", shape=(self.fist_layer_size, self.last_layer_size),
                                        initializer=tf.contrib.layers.xavier_initializer())
            self.u_b2 = tf.get_variable("u_b2", shape=[self.last_layer_size],
                                        initializer=tf.contrib.layers.xavier_initializer())

            self.v_w1 = tf.get_variable("v_w1", shape=(self.num_users, self.fist_layer_size),
                                        initializer=tf.contrib.layers.xavier_initializer())
            self.v_b1 = tf.get_variable("v_b1", shape=[self.fist_layer_size],
                                        initializer=tf.contrib.layers.xavier_initializer())
            self.v_w2 = tf.get_variable("v_w2", shape=(self.fist_layer_size, self.last_layer_size),
                                        initializer=tf.contrib.layers.xavier_initializer())
            self.v_b2 = tf.get_variable("v_b2", shape=[self.last_layer_size],
                                        initializer=tf.contrib.layers.xavier_initializer())
