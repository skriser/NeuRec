...
class JCA(AbstractRecommender):
    def __init__(self, sess, dataset):
        ...
        config.read("conf/JCA.properties")
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):  # The embedding initialization is unknown now
            # user component
            # first layer weights
            self.UV = tf.get_variable(name="UV",
                                      initializer=tf.truncated_normal(shape=[self.num_items, self.hidden_neuron],
                                                                      mean=0, stddev=0.03), dtype=tf.float32)
            # second layer weights
            self.UW = tf.get_variable(name="UW",
                                      initializer=tf.truncated_normal(shape=[self.hidden_neuron, self.num_items],
                                                                      mean=0, stddev=0.03), dtype=tf.float32)
            # first layer bias
            ...
            # item component
            # first layer weights
            self.IV = tf.get_variable(name="IV",
                                      initializer=tf.truncated_normal(shape=[self.num_users, self.hidden_neuron],
                                                                      mean=0, stddev=0.03), dtype=tf.float32)
            # second layer weights
            self.IW = tf.get_variable(name="IW",
                                      initializer=tf.truncated_normal(shape=[self.hidden_neuron, self.num_users],
                                                                      mean=0, stddev=0.03), dtype=tf.float32)
            # first layer bias
            ...
            self.I_factor_vector = tf.get_variable(name="I_factor_vector",
                                                   initializer=tf.random_uniform(shape=[1, self.num_items]),
                                                   dtype=tf.float32)
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            ...
            self.I_Encoder = tool.activation_function(self.g_act,
                                                      I_pre_Encoder * I_pre_mul)  # output of the hidden layer
